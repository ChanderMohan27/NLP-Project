...
def classify():
        if request.method == 'POST':
            ...
            desc_FT = FastText.load("desc_FT.model")
            desc_FT_wv= desc_FT.wv  

            # Generate vector representation of the tokenized data
            desc_FT_dvs = gen_docVecs(desc_FT_wv, [tokenized_data])
            ...
            y_pred = model.predict(desc_FT_dvs)    
            y_pred = y_pred[0] 

            # Set the predicted message
            predicted_message = "The category of this news is {}.".format(y_pred)

            return render_template('admin.html', predicted_message=predicted_message, title=f_title, description=f_content) 
        else:
